...
// suggestion needs explanation 
// Event listener on content-container
tileSection.addEventListener('click', function(e) {
    if(e.target.closest('#heart-icon')) {
        ...
        if(tile) {
            ...
            let donkey = document.getElementById('heart-icon')
            let hasClass = donkey.classList.contains('heart-filled')
            //check classes and apply correct set, increment or decrement
            if (hasClass) {
            posts[arrayPosition].likes -= 1
            donkey.classList.remove('heart-filled')
            donkey.classList.add('heart-standard')
            }
                else { 
                    posts[arrayPosition].likes += 1
                    donkey.classList.add('heart-filled')
                    donkey.classList.remove('heart-standard')
                    donkey.classList.add('bounce')
                }
...
//     //get array position
//     let tileID = document.querySelector(".tile")
//     let arrayPosition = tileID.getAttribute("data-id")
//     console.log(arrayPosition)
...
//     //render
//     let contra = document.querySelector('.likes')
//     contra.innerHTML = `<p class="likes" id="count">${posts[arrayPosition].likes.toLocaleString()} likes</p>`
// })




// let heartIcons = document.querySelector('.heart-active')
// heartIcons.forEach(function(icon) {
//     icon.addEventListener(click, function () {
//         let tile = this.closest('.tile')
//         let arrayPosition = Number(tile.getAttribute('data-id'))
//         posts[arrayPosition].likes += 1

//         let contra = tile.querySelector('.likes')
//         contra.innerHTML = `${posts[arrayPosition].likes.toLocaleString()} likes`
...
// // Get all the elements with the class 'heart-icon'
// const heartIcons = document.querySelectorAll('.heart-icon');

// // Loop over each heart icon and attach the event listener
// heartIcons.forEach(function(icon) {
//     icon.addEventListener('click', function() {
//         // Find the closest parent .tile of the clicked heart icon
//         let tile = this.closest('.tile');
        
//         // Get the array position from the tile's data-id
//         let arrayPosition = Number(tile.getAttribute('data-id'));
        
//         // Increment the likes count in the posts array
//         posts[arrayPosition].likes += 1;
...
// const userAvatar = document.querySelector('.post-avatar')
// function amendAvatar () {
//     userAvatar.style.backgroundImage = `url('${posts[0].post}')`
// }
// amendAvatar ()
